...
| `Acmebot__ExcludeCsrBasicConstraints` | `false` | Remove the Basic Constraints extension from the CSR, for CAs that reject it such as the AWS Certificate Manager ACME endpoint (`badCSR`, "Disallowed extension: 2.5.29.19"). Acmebot rebuilds the Key Vault CSR without the extension and signs it through the Key Vault Keys API, so the private key never leaves Key Vault. Because Key Vault can only sign with the key of a completed certificate version, the key is reused on every renewal, and a new certificate first gets a short-lived self-signed version that holds its key. If issuance fails while the self-signed version is current, automatic renewal retries it at the next check (the daily check for a new certificate, then every six hours) instead of waiting for it to near expiry. While this option is on, the certificate's Reuse key setting is ignored: its Key Vault policy switches to key reuse at the first issuance attempt and keeps it, even after the option is turned off, until the certificate is added again with Reuse key off. Requires the `Key Vault Crypto User` role, or `keys/read` and `keys/sign` permissions, on the vault. |
